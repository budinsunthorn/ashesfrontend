...
import { toggleAnimation, toggleLayout, toggleMenu, toggleNavbar, togglePanel,  toggleRTL, toggleTheme, toggleSemidark, resetToggleSidebar } from '@/store/themeConfigSlice';
    ...
    return (
        <div>
            ...
            <nav
                ...
                } fixed bottom-0 top-0 z-[101] w-full max-w-[400px] bg-white p-4 shadow-[5px_0_25px_0_rgba(94,92,154,0.1)] transition-[right] duration-300 ltr:-right-[400px] rtl:-left-[400px] dark:bg-black`}
            >
                <button
                    type="button"
                    className="absolute bottom-5 flex h-10 w-12 cursor-pointer items-center justify-center bg-primary text-white ltr:-left-12 ltr:rounded-bl-full ltr:rounded-tl-full rtl:-right-12 rtl:rounded-br-full rtl:rounded-tr-full"
                ...
                <div className="perfect-scrollbar h-full overflow-y-auto overflow-x-hidden">
                    ...
                    <div className="mb-3 rounded-md border border-dashed border-white-light p-3 dark:border-[#1b2e4b]">
                        <h5 className="mb-1 text-base leading-none dark:text-white">Panel Type</h5>
                        <p className="text-xs text-white-dark">Box or Plain.</p>
                        <div className="mt-3 flex items-center gap-3 text-primary">
                            <label className="mb-0 inline-flex">
                                <input
                                    type="radio"
                                    checked={themeConfig.panelType === 'box'}
                                    value="panel-type-box"
                                    className="form-radio"
                                    onChange={() => dispatch(togglePanel('box'))}
                                />
                                <span>Box</span>
                            </label>
                            <label className="mb-0 inline-flex">
                                <input
                                    type="radio"
                                    checked={themeConfig.panelType === 'plain'}
                                    value="panel-type-plain"
                                    className="form-radio"
                                    onChange={() => dispatch(togglePanel('plain'))}
                                />
                                <span>Plain</span>
                            </label>
                        </div>
