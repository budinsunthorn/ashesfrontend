...
const BudtenderHeader = () => {
    ...
    const notificationMessages = {
        orderSync: (count: number) =>
            count == 1 ? `There is an unsynced order. Please go to Orders page then check unsynced order.` : `There are ${count} unsynced orders. Please go to Orders page then check unsynced orders.`,
        packageFinish: (count: number) =>
            count == 1 ? `One package is needed to be finished. Please go to Finish Packages page.` : `${count} packages are needed to be finished. Please go to Finish Packages page.`,
        packageReconcile: (count: number) =>
            count == 1 ? `One package is needed to be reconciled. Please go to Metrc Reconciliation page.` : `${count} packages are needed to be reconciled. Please go to Metrc Reconciliation page.`,
        tinyPackage: (count: number) =>
            count == 1
                ? `One package contains very few items. Please go to Tiny Packages page then adjust.`
                : `${count}   packages contain very few items. Please go to Tiny Packages page then adjust.`,
    };
                                                ...
                                                <li key={notification.notifyType + idx} className="dark:text-white-dark/90">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className={`block px-3 py-2 text-sm font-normal dark:text-white-dark text-dark}`}>
                                                            {notificationMessages[notification.notifyType]?.(notification.count) || `You have ${notification.count} notifications.`}
                                                        </span>
                                                    </div>
                                                </li>
                                            );
...
export default BudtenderHeader;
