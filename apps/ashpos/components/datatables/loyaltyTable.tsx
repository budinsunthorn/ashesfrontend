...
import { useAllLoyaltiesByDispensaryIdQuery, useDeleteLoyaltyMutation } from '@/src/__generated__/operations';
...
import { loyaltyTypeArray } from '@/utils/variables';
import TableLoading from '../etc/tableLoading';

type LoyaltyType = 'STANDARD' | 'OTHER';
type RowDataType = {
    id: string;
    name: string;
    type: LoyaltyType;
...
const LoyaltyTable = () => {
    ...
    const deleteLoyaltyMutation = useDeleteLoyaltyMutation();

    const [currentLoyalty, setCurrentLoyalty] = useState<RowDataType>({
        id: '',
        name: '',
        type: 'STANDARD',
    ...
    const allLoyaltiesByDispensaryId = useAllLoyaltiesByDispensaryIdQuery({ dispensaryId: dispensaryId });

    const loyaltyData = allLoyaltiesByDispensaryId.data?.allLoyaltiesByDispensaryId;

    const handleDeleteUser = async (id: string, name: string) => {
        await deleteLoyaltyMutation.mutate(
            {
                id: id,
            },
            {
    ...
    //@ts-expect-error
    const [initialRecords, setInitialRecords] = useState<RowData>(sortBy(loyaltyData, 'name'));
    ...
    useEffect(() => {
        if (loyaltyData) {
            //@ts-expect-error
            setInitialRecords(() => {
                return loyaltyData?.filter((item) => {
                    return (
    ...
    return (
        <div className={`panel mt-6 pt-2 ${panelType == 'plain' ? 'plain-panel' : ''}`}>
            <div className="my-2 lg:flex flex-col gap-5 md:sm:flex-row md:items-center">
                <h5 className="text-lg font-semibold dark:text-white-dark">Loyalty</h5>
                <div className="lg:flex items-center gap-2 ltr:ml-auto rtl:mr-auto">
                    ...
                    <RefreshButton onClick={() => allLoyaltiesByDispensaryId.refetch()}/>
...
<BsColumns className="text-xl" />
                        ...
                        <ExportTable cols={cols} recordsData={recordsData} hideCols={hideCols} filename='loyalty_table_data'/>
                    </div>
            ...
            <div className="datatables">
                <DataTable
                    ...
                    records={recordsData ?? []}
                    fetching={allLoyaltiesByDispensaryId.isLoading || allLoyaltiesByDispensaryId.isFetching}
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            render: ({ type }) => <div>{loyaltyTypeArray[type]}</div>,
                        },
                        {
                            accessor: 'isActive',
                            title: 'Active',
                        ...
                        },
                        {
                            ...
                            title: 'Action',
                            sortable: false,
                            render: ({ id, name, type, applyDurationSet, applyTo, applyFrom, isActive, pointWorth, isAdminPin, color, createdAt, updatedAt }) => (
                                <div>
                                    <Tippy content="Edit">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                setModalMode('update');
                                                setCurrentLoyalty({ id, name, type, applyDurationSet, applyTo, applyFrom, isActive, pointWorth, isAdminPin, color, createdAt, updatedAt });
                                                setModalShow(true);
                                            }}
                                        >
            ...
            <LoyaltyModal setModalShow={setModalShow} modalShow={modalShow} modalMode={modalMode} dispensaryId={dispensaryId} currentLoyalty={currentLoyalty} />
        </div>
    );
};
