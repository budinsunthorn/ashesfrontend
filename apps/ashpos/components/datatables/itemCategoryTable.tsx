...
import { useAllItemCategoriesByDispensaryIdQuery, useDeleteItemCategoryMutation, PurchaseLimitType } from '@/src/__generated__/operations';
import ItemCategoryRegisterModal from '@/components/modals/itemCategoryRegisterModal';
...
import {purchaseLimitTypeArray} from '@/utils/variables';
...
const ItemCategoryTable = () => {
    ...
    const deleteItemCategoryMutation = useDeleteItemCategoryMutation();
    const allItemCategoriesByDispensaryId = useAllItemCategoriesByDispensaryIdQuery({ dispensaryId: dispensaryId });

    const itemCategoryData = allItemCategoriesByDispensaryId.data?.allItemCategoriesByDispensaryId;

    const handleDeleteItemCategory = async (id: string, name: string) => {
        await deleteItemCategoryMutation.mutate(
            {
                id: id,
            },
            {
    ...
    const deleteAlert = async (id: string, name: string) => {
        Swal.fire({
            ...
            if (result.value) {
                handleDeleteItemCategory(id, name);
            } else {
            }
        });
    };
    ...
    const [currentItemCategory, setCurrentItemCategory] = useState<RowDataType>({
        id: '',
        name: '',
        color: '#64748b',
        metrcCategory: '',
        purchaseLimitType: 'Clone',
        containMj: false,
        createdAt: '',
        updatedAt: '',
    });
            ...
            setInitialRecords(() => {
                return itemCategoryData?.filter((item) => {
                    return (
                        ...
                        item?.dispensaryId?.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (
        <div className={`panel mt-6 pt-2 ${panelType == "plain" ? "plain-panel" : ""}`}>
            <div className="my-2 flex flex-col gap-5 md:flex-row md:items-center">
                <h5 className="text-lg font-semibold dark:text-white-dark">Product Category</h5>
                <div className="lg:flex items-center gap-2 lg:ltr:ml-auto rtl:mr-auto">
                    <div>
                        <button type="button" onClick={handleNewItemCategory} className="btn btn-primary rounded-full py-1.5 px-3 !text-sm">
                            <IconPlus className="h-5 w-5 shrink-0 ltr:mr-1.5 rtl:ml-1.5" />
                            New
                        </button>
                    </div>
                    <div className="flex flex-col gap-2 md:flex-row md:items-center">
                    <RefreshButton onClick={() => allItemCategoriesByDispensaryId.refetch()}/>
...
<BsColumns className="text-xl" />
                    ...
                    <ExportTable cols={cols} recordsData={itemCategoryData} hideCols={hideCols} filename='itemCategory_table_data'/>
                    </div>
            ...
            <div className="datatables">
                <DataTable
                    ...
                    records={recordsData ?? []}
                    fetching={allItemCategoriesByDispensaryId.isLoading || allItemCategoriesByDispensaryId.isFetching}
                    ...
                    columns={[
                        ...
                        },
                        {
                            ...
                            title: 'Contains MJ',
                            sortable: true,
                            hidden: hideCols.includes('containMj'),
                            render: ({ containMj }) => (
                                <span
                                    className={`${containMj ? 'bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-300'} `}
                                >
                                    {containMj ? 'yes' : 'no'}
                                </span>
                            ),
                        ...
                        {
                            accessor: 'metrcCategory',
                            title: 'Metrc Category',
                            sortable: true,
                            hidden: hideCols.includes('metrcCategory'),
                            render: ({ containMj, metrcCategory }) => (containMj ? metrcCategory : ''),
                        },
                        {
                            accessor: 'purchaseLimitType',
                            title: 'Limit Type',
                            sortable: true,
                            hidden: hideCols.includes('purchaseLimitType'),
                            render: ({ purchaseLimitType }) => (<div>{purchaseLimitType}</div>),
                            // render: ({ purchaseLimitType }) => {
                            //     const validKey = purchaseLimitType in purchaseLimitTypeArray;
                            //     console.log("validkey", validKey);
                            //     return (
                            //         <div>
                            //         {validKey ? purchaseLimitTypeArray[purchaseLimitType as keyof typeof purchaseLimitTypeArray] : ''}
                            //         </div>
                            //     );
                            // },
                        },
                        ...
                        },
                        {
                            ...
                            title: 'Action',
                            sortable: false,
                            render: ({ id, name, color, metrcCategory, purchaseLimitType, containMj, createdAt, updatedAt }) => (
                                <div>
                                    <Tippy content="Edit">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                setModalMode('update');
                                                setCurrentItemCategory({ id, name, color, metrcCategory, purchaseLimitType, containMj, createdAt, updatedAt });
                                                setModalShow(true);
                                            }}
                                        >
            ...
            <ItemCategoryRegisterModal setModalShow={setModalShow} modalShow={modalShow} modalMode={modalMode} currentItemCategory={currentItemCategory} dispensaryId={dispensaryId} />
        </div>
    );
};
