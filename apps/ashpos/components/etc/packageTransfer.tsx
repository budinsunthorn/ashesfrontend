...
export default function PackageTransfer({ packageData, setIsUpdate, transferId, mtcTransferId, isMj }: any) {
    ...
    useEffect(() => {
        ...
        setSearchValue(isMj ? packageData?.package?.assignPackage?.product?.name || '' : packageData?.product?.name);
        setCurrentProduct({ id: isMj ? packageData?.package?.assignPackage?.productId : packageData?.assignPackage?.productId });
        setProductUnit(isMj ? packageData?.package?.assignPackage?.product?.productUnitOfMeasure || '' : packageData?.assignPackage?.product?.productUnitOfMeasure || '');

        if (!packageData.package?.isConnectedWithProduct && isMj) {
            setQuantity(packageData?.package?.originalQty);
        }
        ...
        // }
        const isSelled =
            (isMj && packageData.package?.assignPackage && packageData?.package?.originalQty > packageData?.package?.assignPackage?.posQty) ||
            (!isMj && packageData?.assignPackage && packageData?.originalQty > packageData?.assignPackage?.posQty);
        const isAssigned = (isMj && packageData.package?.assignPackage) || (!isMj && packageData?.assignPackage);
    ...
    const handleAssignPackage = async () => {
        ...
        Swal.fire({
            icon: 'warning',
            title: '<span class="xtext-dark dark:text-white-dark">Do you really want this connection?</span>',
            width: "600px",
            html: `<div class="flex flex-col justify-center items-center text-dark dark:text-white-dark my-6">
                    <div class="w-full flex flex-col justify-between mb-4">
                    <div class="flex justify-between items-center">
                    <span>Package:</span>
                    <span class="badge text-primary dark:bg-primary-dark bg-primary-light ml-2 block text-[15px]">${packageData.package?.itemName}</span>
                    </div>
                    <span class="text-sm text-right">${packageData?.packageLabel}</span>
                    </div>

                    <div class="w-full mt-2 flex justify-between items-center">
                    <span>Product:</span>
                    <span class="badge text-success bg-success-light dark:bg-success-dark-light ml-2 text-[15px]">${searchValue}</span>
                    </div>
                    </div>`,
                ...
                await assignPackage.mutate(
                    {
                        input: {
                            ...
                            cost: Number(truncateToTwoDecimals(cost as number)),
    ...
    const handleNonMjAssignPackage = async () => {
        ...
        Swal.fire({
            icon: 'warning',
            title: '<span class="text-3xl text-dark dark:text-white-dark">Do you really want this connection?</span>',
            width: "600px",
            html: `<div class="flex flex-col justify-center items-center text-dark dark:text-white-dark my-6">
                    <div class="w-full flex flex-col justify-between mb-4">
                    <div class="flex justify-between items-center">
                    <span>Package:</span>
                    <span class="badge text-primary dark:bg-primary-dark bg-primary-light ml-2 block text-[15px]">Non MJ</span>
                    </div>
                    </div>
    ...
    return (
        <div
            className={`bg-white dark:bg-[#0f1727] rounded-lg shadow-md p-6 mx-auto mb-2 ${
                isMj && packageData?.package?.assignPackage ? '' : packageData?.assignPackage ? '' : '!border-danger !border-2'
            }`}
        >
            ...
            <div className="flex justify-between items-center mb-4">
                <div className="flex justify-start items-center">
                    ...
                    {isMj ? <PackageStatusBadge packageStatus={packageData?.package?.packageStatus} /> : <PackageStatusBadge packageStatus={packageData?.packageStatus} />}
                ...
                {isAssigned && isSelled ? (
                    <span className="badge text-success bg-success-light dark:bg-success-dark-light rounded-lg">Selling Started</span>
                ) : isAssigned && !isSelled ? (
                    <button
            ...
            {isMj ? (
                <div className="grid grid-cols-2 gap-2 mb-2">
                    ...
                    <div className="flex flex-row justify-start items-center">
                        ...
                        {/* <ProductCategory name={packageData.package?.itemProductCategoryName} color={packageData.package?.color || '#d8dad9'}/> */}
                    </div>
                    <div className="flex flex-row justify-start items-center">
                        ...
                        <p className="text-sm text-dark dark:text-white-dark">
                            {packageData.package?.originalQty}&nbsp;{quantityAbbreviations[packageData.package?.UnitOfMeasureAbbreviation]}
                        </p>
                    </div>
                    {/* <div className="flex flex-row justify-start items-center">
            ...
            <div className="w-full flex my-4">
                ...
                <div className="w-[40%] px-2">
                    ...
                    <div className="flex w-full">
                        <div className={`relative border-[1] border-gray-300 rounded-r-none w-full ${isShowError && searchValue == '' ? 'has-error' : ''}`}>
                            ...
                            {showProduct ? (
                                <button
                                    type="button"
                                    className="absolute top-1/2 right-3 -translate-y-1/2 btn btn-outline-primary btn-sm ml-2 rtl:mr-2 p-2 shadow shadow-indigo-500/50"
                                    onClick={() => {
                                            ...
                                            productData.map((item, i) => (
                                                ...
                                                >
                                                    ...
                                                    <span className="text-xs">
                                                        1{item?.productUnitOfMeasure}&nbsp;&nbsp;${item?.price}
                                                    </span>
                                                </div>
                                            ))
                                        ) : (
                    ...
                    <p className="text-sm py-2">
                        ${truncateToTwoDecimals(price)} Net Price | {packageData.margin} | ${packageData.package?.isConnectedWithProduct ? price - packageData.package?.cost : price - cost} Profit/Sale
                    </p>
                </div>
                <div className="w-[20%] px-2">
                    ...
                    <div className={`flex  ${isShowError && (quantity == 0 || quantity == undefined) ? 'has-error' : ''}`}>
                        <input
                            ...
                            value={quantity}
                            disabled={isAssigned || isSelled}
                            readOnly={isMj ? packageData?.package?.packageStatus == 'Finished' : packageData?.packageStatus == 'Finished'}
       ...
       <div>
        ...
        <div>
          ...
          <div className="flex items-center border rounded mt-1">
            ...
            <ProductRegisterModal setModalShow={setModalShow} modalShow={modalShow} modalMode={'new'} currentProduct={currentProductData} userId={userId} dispensaryId={dispensaryId} />
        </div>
    );
}
